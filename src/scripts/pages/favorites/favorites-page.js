...
const FavoritesPage = {
      ...
      container.innerHTML = favorites.map(favorite => `
        <article class="story-card favorite-card">
          <div class="story-header">
            ...
            <button class="remove-favorite-btn" data-story-id="${favorite.storyId}"
          ...
          <div class="story-content">
            ...
            <div class="story-meta">
              <small>Ditambahkan: ${new Date(favorite.addedAt).toLocaleDateString('id-ID')}</small>
            </div>
          </div>
        </article>
      `).join('');
  ...
  _setupFavoriteInteractions() {
    document.querySelectorAll('.remove-favorite-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        this._loadFavorites(); // Reload the list
      });
    });
  },
  ...
  async _exportFavorites() {
    try {
      ...
      a.href = url;
      a.download = `cerita-favorit-${new Date().toISOString().split('T')[0]}.json`;
